...
from ..graph.connection import TraverseNotInstalled
...
def cmd_build(kb: Path = _KB_OPT, json_output: bool = _JSON_OPT) -> None:
    ...
    except (FileNotFoundError, TraverseNotInstalled, EmbedderError, IndexError_) as exc:
...
def search_command(
    ...
    except (FileNotFoundError, TraverseNotInstalled, EmbedderError) as exc:
        _fail(str(exc), json_output)
        return
    if json_output:
