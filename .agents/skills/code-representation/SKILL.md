...
*   **Symbolic-Equation Edge Discipline**: When linking an `Equation` node to `Quantity` nodes in the graph:
    ...
    *   `USES_SYMBOL` (`(Equation)-[:USES_SYMBOL]->(Quantity)`): Connect all input terms, intermediate parameters, normalization constants, and sub-expression symbols appearing inside the expression or formula.
