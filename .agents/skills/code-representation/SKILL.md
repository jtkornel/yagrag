...
2.  **Write the Snippet**: Create a file under the `code/` directory of the knowledge base.
    *   For **SymPy**: Use a `.sympy` extension. Write one relation per line (e.g., `Eq(y, x + 1)`). Reuse symbols already present on symbol-bearing nodes in the graph. Ensure all free symbols (including intermediate parameters, normalization constants, and sub-expression terms like $C$, $N$, $\theta_k$) have corresponding `Quantity` nodes in the graph linked via `USES_SYMBOL` edges to maximize symbol-graph interconnectivity and resolve symbol checks cleanly.
...
*   **Symbolic-Equation Edge Discipline**: When linking an `Equation` node to `Quantity` nodes in the graph:
    *   `EXPRESSED_BY` (`(Quantity)-[:EXPRESSED_BY]->(Equation)`): **Mandatory** link connecting the target output quantity on the left-hand side (LHS) formulated by the equation.
    *   `DEFINED_BY` (`(Quantity)-[:DEFINED_BY]->(Equation)`): **Optional refinement** on top of `EXPRESSED_BY` to designate an authoritative or axiomatic definition.
    *   `USES_SYMBOL` (`(Equation)-[:USES_SYMBOL]->(Quantity)`): **Mandatory** link connecting all input terms, intermediate parameters, normalization constants, and sub-expression symbols appearing inside the expression, RHS, or inequality constraint.
...
*   **Symbol Consistency**: In SymPy snippets, prefer symbols that match the `symbol`, `id` or `name` of a symbol-bearing node. All free symbols—including intermediate parameters, normalization constants, scale factors, and sub-expression terms (e.g., $C$, $N$, $\theta_k$)—should correspond to explicit `Quantity` nodes in the graph connected via `USES_SYMBOL` edges. An unrecognised symbol is reported as a *warning* only — if the symbol is legitimate, extract the missing symbol-bearing `Quantity` node and link it to achieve zero-warning static checks and maximize symbol-graph interconnectivity.
