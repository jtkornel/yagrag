...
- **High-Fidelity Document & Diagram Extraction (Docling)**: Direct integration with Docling for structural layout analysis, table extraction (TableFormer), mathematical formula recovery (LaTeX), and diagram/figure cropping. Extracted content, ASTs, and figures are cached on disk (`documents/cache/<doc_id>/`) with SHA-256 validation to eliminate repetitive layout inference overhead. For architectural details and visual grounding workflows, see [`docs/proposal_document_and_diagram_extraction.md`](proposal_document_and_diagram_extraction.md).
